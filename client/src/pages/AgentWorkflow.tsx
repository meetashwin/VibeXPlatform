...
import WorkflowTemplates, { WorkflowTemplate } from '@/components/agents/WorkflowTemplates';
...
const AgentWorkflow: React.FC = () => {
  ...
  const updateNode = () => {
    ...
    toast({
      title: "Agent Updated",
      description: `Updated agent: ${editNodeName}`,
    });
  };
  ...
  const updateNodeConfig = () => {
    ...
    toast({
      ...
      description: `Updated agent configuration for: ${selectedNode.data.label}`,
    });
  };
  ...
  const executeWorkflow = () => {
    ...
    // Reset all nodes to inactive state at the start
    setNodes(nds => nds.map(node => ({
      ...node,
      data: {
        ...node.data,
    ...
    const processNextNode = () => {
      if (currentIndex >= nodes.length) {
        // All nodes processed - set all nodes to active but not processing
        setNodes(nds => nds.map(node => ({
          ...node,
          data: {
            ...node.data,
      ...
      // Set current node to processing state
      setNodes(nds => nds.map(node => ({
        ...node,
        data: {
          ...node.data,
          isActive: node.id === currentNode.id || node.data.isActive,
      ...
      // Randomly decide if this node will have an error (10% chance for demo)
        ...
        if (hasError) {
          ...
          setNodes(nds => nds.map(node => ({
            ...
            data: {
              ...
              isActive: node.id === currentNode.id || node.data.isActive,
              isProcessing: false,
              isError: node.id === currentNode.id,
              onEdit: () => handleEditNode(node.id),
              onConfigure: () => handleConfigureNode(node.id)
            }
          })));
          ...
          toast({
            ...
            description: `${currentNode.data.label} encountered an issue!`,
            variant: "destructive",
          });
          
          // Pause for a moment to show the error
          setTimeout(() => {
            // Retry the node (for demo purposes, we'll just act like it succeeded the second time)
            setNodes(nds => nds.map(node => ({
              ...node,
              data: {
                ...node.data,
            ...
            toast({
              title: "Retrying Agent",
              description: `Retrying ${currentNode.data.label}...`,
            });
            ...
            setTimeout(() => {
              ...
              // Update node state to active but not processing
              setNodes(nds => nds.map(node => ({
                ...node,
                data: {
                  ...node.data,
          ...
          // Mark this node as completed (no errors)
          setExecutionProgress(prev => 
            prev.map(item => 
              item.id === currentNode.id 
                ? { ...item, completed: true } 
        ...
        <div className="w-64 bg-white border-r-4 border-black p-4 flex flex-col">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Button
              ...
              className="border-2 border-black brutal-shadow brutal-button"
              onClick={() => {
                // Create default nodes with handlers
                const defaultNodes = defaultInitialNodes.map(node => ({
                  ...node,
                  data: {
                    ...node.data,
                    onEdit: () => handleEditNode(node.id),
            ...
            {/* Add the WorkflowTemplates component */}
            <Separator className="my-2 border-black" />
            <h3 className="font-bold">Templates</h3>
            <WorkflowTemplates 
              onSelectTemplate={(template: WorkflowTemplate) => {
                // Add handlers to template nodes
                const nodesWithHandlers = template.nodes.map((node: Node) => ({
                  ...node,
                  data: {
                    ...node.data,
                    onEdit: () => handleEditNode(node.id),
                ...
                toast({
                  ...
                  description: `Applied the "${template.name}" template to your workflow`,
                });
              }}
            />
            ...
            <Button
              variant="outline"
              className="border-2 border-black bg-green-100 brutal-shadow brutal-button mt-2"
